...
class HeartsGambitPlugin implements GamePlugin {
  ...
  private startNewRound(room: Room) {
    ...
    if ((!gameState.currentTurn || !room.players.get(gameState.currentTurn)?.connected) && activePlayers.length > 0) {
  ...
  private checkRoundEnd(room: Room): boolean {
      ...
      if (gameState.deck.length === 0) {
          ...
          if (winners.length > 1) {
             ...
             const getDiscardSum = (p: Player) => (p.gameData as HeartsGambitPlayerData).discarded.reduce((a,b)=>a+b, 0);
