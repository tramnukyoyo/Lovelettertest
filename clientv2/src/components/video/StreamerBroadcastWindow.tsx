 ...
 *
 * Mirrors the Soundbite / CanvasChaos pattern: wires to VideoUIContext so
 * `requestStreamerBroadcast()` from anywhere in the app opens the popup.
 */
...
const StreamerBroadcastWindow: React.FC<StreamerBroadcastWindowProps> = ({
  ...
  const { setStreamerBroadcastOpen, setOnBroadcastRequested } = useVideoUI();
  ...
  const openBroadcast = useCallback(() => {
    ...
    const accentColor = getComputedStyle(document.documentElement).getPropertyValue('--accent-color') || '#ffb648';
    const panelBg = getComputedStyle(document.documentElement).getPropertyValue('--panel-bg') || '#3b2417';
    const textPrimary = getComputedStyle(document.documentElement).getPropertyValue('--text-primary') || '#f7e7c5';
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body {
      /* Nunito arrives with the cloned stylesheets + <base href>; system-ui
         only covers the pre-clone flash. */
...
<body>
  ...
  }, [copy.content.windowTitle, onOpen, onClose, setStreamerBroadcastOpen]);
