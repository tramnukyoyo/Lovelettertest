  ...
  cfFetchInFlight = fetch(url, { credentials: 'omit' })
    .then(async (resp) => {
      if (!resp.ok) {
        ...
        if (resp.status !== 503) {
          console.warn(`[Video/webrtc] TURN credential fetch returned ${resp.status}`);
        }
        return;
      }
      ...
      console.log('[Video/webrtc] ✅ Cloudflare TURN credentials cached');
    })
    .catch((err) => {
      console.warn('[Video/webrtc] TURN credential fetch failed (will use Metered fallback):', err?.message || err);
    })
    .finally(() => {
...
export function getICEServers(): RTCIceServer[] {
  ...
  if (!cfRuntimeCache && !metered) {
    console.warn('[Video/webrtc] ⚠️ No TURN servers configured — cellular/CGNAT players will fail to connect');
  }
...
export async function setMaxVideoBitrate(pc: RTCPeerConnection, kbps: number): Promise<void> {
  ...
  for (const sender of senders) {
    try {
      ...
      console.warn(`[Video/webrtc] Failed to cap video bitrate to ${kbps}kbps:`, err);
    }
  }
}
...
export function setH264CodecPreference(peerConnection: RTCPeerConnection, peerId: string): void {
  ...
  console.log(`[Video/webrtc] iOS device detected, setting H.264 codec preference for ${peerId}`);
  ...
  try {
    ...
    console.warn(`[Video/webrtc] Failed to set H.264 codec preference for ${peerId}:`, error);
  }
}
...
export function addEnhancedDiagnostics(peerConnection: RTCPeerConnection, peerId: string): void {
  peerConnection.oniceconnectionstatechange = () => {
    ...
    console.log(`[Video/webrtc] ICE connection state with ${peerId}: ${iceState}`);

    if (iceState === 'failed') {
      console.error(`[Video/webrtc] ICE connection failed with ${peerId}`);
    }
  };

  peerConnection.onicegatheringstatechange = () => {
    console.log(`[Video/webrtc] ICE gathering state with ${peerId}: ${peerConnection.iceGatheringState}`);
  };

  peerConnection.onconnectionstatechange = () => {
    console.log(`[Video/webrtc] Connection state with ${peerId}: ${peerConnection.connectionState}`);
  };
}
...
export async function getUserMediaWithFallback(options: GetUserMediaOptions = {}): Promise<GetUserMediaResult> {
  ...
  // Try video + audio
  try {
    console.log('[Video/webrtc] Attempting video + audio...');
    ...
    console.log('[Video/webrtc] Video + Audio enabled');
    return { stream, hasVideo, hasAudio };
  } catch (error) {
    console.log('[Video/webrtc] Video + Audio failed, trying audio-only...', error);
  }
  ...
  try {
    ...
    const hasAudio = stream.getAudioTracks().length > 0;
    console.log('[Video/webrtc] Audio-only enabled');
    return { stream, hasVideo: false, hasAudio };
  } catch (error) {
    console.log('[Video/webrtc] Audio failed, trying video-only...', error);
  }
  ...
  try {
    ...
    const hasVideo = stream.getVideoTracks().length > 0;
    console.log('[Video/webrtc] Video-only enabled');
    return { stream, hasVideo, hasAudio: false };
  } catch (error) {
    console.log('[Video/webrtc] All media access failed', error);
  }
